...
class Wav2VecConfig(FairseqDataclass):
    ...
    conv_aggregator_layers: str = field(
        default="[(256, 1, 1), (256, 2, 1), (256, 3, 1), (256, 4, 1), (256, 5, 1), (256, 6, 1), (256, 7, 1), (256, 8, 1), (256, 9, 1), (256, 10, 1), (256, 11, 1), (256, 12, 1), (256, 13, 1)]",
        metadata={
...
class Wav2VecModel(BaseFairseqModel):
    ...
    def __init__(self, cfg: Wav2VecConfig):
        ...
        self.feature_aggregator_bwd, _ = make_aggregator()
    ...
    def forward(self, source):
        ...
        # Backward Training
        source_r = source.cpu().numpy()[::-1].copy()
        source_r = torch.from_numpy(source_r)
        features_reverse = self.feature_extractor(source_r.cuda())
        if self.vector_quantizer:
            q_res = self.vector_quantizer(features_reverse)
            features_reverse = q_res["x_reverse"]
            for k in q_res.keys():
                if k != "x_reverse":
                    result[k] = q_res[k]

        x_reverse = self.dropout_feats(features_reverse)
        x_reverse = self.feature_aggregator_bwd(x_reverse)
        x_reverse = self.dropout_agg(x_reverse)

        if self.project_features is not None:
            features_reverse = self.project_features(features_reverse)
        x_reverse, targets = self.wav2vec_predictions(x_reverse, features_reverse)
        result["cpc_logits_bwd"] = x_reverse
        result["cpc_targets_bwd"] = targets
    ...
    def get_logits(self, net_output):
        logits, logits_bwd = net_output["cpc_logits"],net_output["cpc_logits_bwd"]
        return logits.float(), logits_bwd.float()

    def get_targets(self, sample, net_output):
        t, t_bwd = net_output["cpc_targets"], net_output["cpc_targets_bwd"]
        if isinstance(t, tuple):
            t, t_bwd = t[0], t_bwd[0]
        return t.contiguous(), t_bwd.contiguous()

    def get_target_weights(self, targets, net_output):
        targets = net_output["cpc_targets"]
        targets_bwd = net_output["cpc_targets_bwd"]
        if isinstance(targets, tuple) and targets[-1] is not None:
            return targets[-1], targets_bwd[-1]
        return None
