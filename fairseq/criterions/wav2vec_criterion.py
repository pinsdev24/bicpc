...
class Wav2vecCriterion(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        logits, logits_bwd = model.get_logits(net_output)
        target, target_bwd = model.get_targets(sample, net_output)
        self.xla = is_xla_tensor(logits)

        # XXX: handle weights on xla.
        weights, weights_bwd = None, None
        if hasattr(model, "get_target_weights") and not self.infonce:
            weights = model.get_target_weights(target, net_output)
            weights_bwd = model.get_target_weights(target_bwd, net_output)
            if torch.is_tensor(weights) and torch.is_tensor(weights_bwd):
                weights, weights_bwd = weights.float(), weights_bwd.float()

        losses = []

        reduction = "none" if ((not reduce) or self.xla) else "sum"
        if self.infonce:
            loss1 = F.cross_entropy(logits, target, reduction=reduction)
            loss2 = F.cross_entropy(logits_bwd, target_bwd, reduction=reduction)
            loss = loss1 + loss2
        else:
            ...
            loss2 = F.binary_cross_entropy_with_logits(
                logits_bwd, target_bwd.float(), weights_bwd, reduction=reduction
            )
            loss = loss1 + loss2
